...
#modified by Mike Connelly, 1/12/22

#PARSE ARUGMENTS
library(optparse)
option_list = list(
  
  make_option(c("--i"), type="character", default="^*_Pdam.counts$", 
              help="glob to infiles"),
  
  make_option(c("--pth"), type="character", default='./outputs/featureCounts_pdam/', 
...
#get files names
fileList = list.files(path = "./outputs/featureCounts_pdam/", pattern = "*counts")
