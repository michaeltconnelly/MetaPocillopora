...
## Import VCF file
```{r imporf vcf}
vcf_meta <- vcfR::read.vcfR(file = "./outputs/phylotrans_pdam/filtered_vcfs/max-missing-0.75.recode.vcf")
```
...
## PCA
```{r}
grp_meta <- find.clusters(genind_meta, max.n.clust = 40)
```
```{r}
pdf("./outputs/adegenet/pca_find-clusters.pdf", height = 5, width = 6)
grp_meta <- find.clusters(genind_meta, max.n.clust = 40, n.pca = 81)
dev.off()
```
```{r}
...
```
```{r}
pdf("./outputs/adegenet/pca_max-missing-0.25_indv-missing_0.95_maf-0.01_hwe-0.05.pdf")
pca_df %>%
  ggplot(., aes(Axis1, Axis2)) +
  geom_point(size = 3, alpha = 0.5) + #aes(color = Site, shape = Species),
...
# 
samples_keep <- samples_all %>% 
  filter(Sample_ID %in% samples_grp$Sample_ID) %>% 
  left_join(samples_grp) %>% 
  arrange(Sample_ID) %>% select(-Number, -Dataset, -SampleID)
# 
