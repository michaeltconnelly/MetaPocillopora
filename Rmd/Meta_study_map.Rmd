...
map <- study_locations %>% 
  ...
  scale_fill_manual(values = gcolors,
                    guide = guide_legend(order = 1, override.aes = list(size = 3, shape = 21, fill = gcolors), nrow = 5, title.position = "top")) +
  scale_size(range = c(1, 8), breaks = c(1, 12, 24, 48, 60), limits = c(0, 60),
...
map <- study_locations %>% 
  ...
  scale_size(range = c(1, 12), breaks = c(1, 12, 24, 48, 60), limits = c(0, 60),
             guide = guide_legend(order = 2, override.aes = list(fill = "black"), nrow = 5, title.position = "top")) +
  # 
  coord_sf(ylim = c(-42, 42), xlim = c(20, 295), expand = F) +
  # 
...
# pdf("./outputs/figures/Fig1_study_locations_map.pdf", width = 6.5, height = 4, units = "in")
